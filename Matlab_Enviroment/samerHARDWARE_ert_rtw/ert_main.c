...
int main(void)
{
  ...
  init();
  MW_Arduino_Init();
  rtmSetErrorStatus(samerHARDWARE_M, 0);
  ...
  samerHARDWARE_initialize();
  cli();
  sei();

  /* External Mode initialization */
  errorCode = extmodeInit(samerHARDWARE_M->extModeInfo, (extmodeSimulationTime_T
    *)rteiGetPtrTFinalTicks(samerHARDWARE_M->extModeInfo));
  if (errorCode != EXTMODE_SUCCESS) {
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(samerHARDWARE_M, true);
    }
  }
  ...
  runModel =
    ...
    !rtmGetStopRequested(samerHARDWARE_M);
