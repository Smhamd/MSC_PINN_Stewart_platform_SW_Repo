...
int main(void)
{
  ...
  init();
  MW_Arduino_Init();
  rtmSetErrorStatus(samerHARDWAREPOTread_M, 0);
  ...
  samerHARDWAREPOTread_initialize();
  cli();
  sei();

  /* External Mode initialization */
  errorCode = extmodeInit(samerHARDWAREPOTread_M->extModeInfo,
    (extmodeSimulationTime_T *)rteiGetPtrTFinalTicks
    (samerHARDWAREPOTread_M->extModeInfo));
  if (errorCode != EXTMODE_SUCCESS) {
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(samerHARDWAREPOTread_M, true);
    }
  }
  ...
  runModel =
    ...
    !rtmGetStopRequested(samerHARDWAREPOTread_M);
  ...
  /* Terminate model */
  samerHARDWAREPOTread_terminate();
